...
class ChangelogCI:
    ...
    def _get_latest_release_date(self):
        ...
        tag_times = subprocess.check_output(['git', 'tag', '-l', '--sort=-creatordate', '--format=%(creatordate)']).decode("utf-8").splitlines()
        latest_tag_time = tag_times[0].strip()

        from datetime import datetime

        result = datetime.strptime(latest_tag_time, "%a %b %d %H:%M:%S %Y %z")

        return result

    def _get_last_release_date(self):
        import subprocess
        ...
        last_tag_time = tag_times[1].strip()

        from datetime import datetime

        result = datetime.strptime(last_tag_time, "%a %b %d %H:%M:%S %Y %z")
    ...
    def _get_pull_requests_after_last_release(self):
        ...
        last_release_date_timestamp = 0
        if last_release_date:
            last_release_date_timestamp = last_release_date.timestamp()

        latest_release_date = self._get_latest_release_date()

        url = 'https://api.github.com/repos/{repo_name}/pulls?state=closed&base=main&per_page=100'.format(
        ...
        if response.status_code == 200:
            ...
            for item in response_data:
                ...
                if last_release_date_timestamp - created_at.timestamp() >= ninety_days:
                    break
                ...
                if merged_at.timestamp() <= last_release_date_timestamp or merged_at.timestamp() > latest_release_date.timestamp():
                    continue
